...
describe("SearchResults page", () => {
  ...
  it("renders error message on fetch failure", async () => {
    ...
    vi.spyOn(api, "getWorkpodCalendar").mockImplementation(() =>
      Promise.resolve([])
    );
    vi.spyOn(api, "getWorkpodCalendar").mockRejectedValueOnce(
  ...
  it("renders current time in the header", async () => {
    ...
    const expectedText = `searchresults-available ${format(
      now,
      "dd/MM/yyyy HH:mm"
    )}`;
  ...
  it("renders an available workpod", async () => {
    ...
    vi.spyOn(api, "getWorkpodCalendar").mockImplementation((workpodId) => {
      if (workpodId === (data![0].alias || "Room1-A")) {
        // ensure Room1-A is available
