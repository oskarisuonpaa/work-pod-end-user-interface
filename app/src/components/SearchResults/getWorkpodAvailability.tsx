import { differenceInMinutes, isWithinInterval, setHours, setMinutes, add } from "date-fns";
...
export function getWorkpodAvailability(events: DataItem[], date: Date): WorkpodAvailability {
    let freeUntil = null;
    let freeFor = 0;
    ...
    if (!isReserved) {
        ...
        const nextReservation = sortedEvents.find((event) => new Date(event.start) > date);
